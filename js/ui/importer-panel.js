...
import {
  BUILTIN, builtinSource, getImporter, setImporter, clearImporter,
  tryImporter, importerPrompt, cleanGeneratedCode,
...
export async function openImporterPanel(formatId = state.formatId) {
  ...
  overlay.append(el('div', { class: 'modal wide' },
...
async function render(body, formatId) {
  ...
  const picker = el('div', { class: 'importer-row' },
    el('span', { class: 'settings-label' }, t('imp.startFrom')),
    ...BUILTIN.map((b) => el('button', {
      class: 'btn sm ghost',
      onclick: async () => { code.value = await builtinSource(b.file); status.textContent = t('imp.loaded', { name: b.label }); },
    }, b.label)));

  /* ---- or describe the files and have one drafted ---- */
  const describe = el('textarea', { class: 'edit-box', rows: 2, placeholder: t('imp.describeHint') });
  const sample = el('textarea', { class: 'edit-box', rows: 4, spellcheck: 'false', placeholder: t('imp.sampleHint') });
  const draftBtn = el('button', { class: 'btn sm', onclick: () => draft() }, t('imp.draft'));

  async function draft() {
    const provider = state.provider;
    const key = (state.apiKeys[provider] || '').trim();
    if (!key) { status.textContent = t('app.needKey'); status.className = 'edit-status err'; return; }
    status.textContent = t('imp.drafting'); status.className = 'edit-status';
    try {
      ...
      status.textContent = t('imp.drafted'); status.className = 'edit-status ok';
    } catch (err) {
  ...
  async function dryRun() {
    ...
    if (!text) { status.textContent = t('imp.needSample'); status.className = 'edit-status err'; return; }
    status.textContent = t('imp.running'); status.className = 'edit-status';
    try {
      const doc = await tryImporter(code.value, text, 'sample.txt');
      status.textContent = t('imp.ranOk', { n: doc.sentences.length }); status.className = 'edit-status ok';
      mount(preview,
        el('div', { class: 'hint' }, t('imp.previewTitle', { n: doc.sentences.length, lang: doc.language })),
        ...doc.sentences.slice(0, 6).map((s) => el('div', { class: 'importer-sent' },
          el('span', { class: 'tok-idx' }, String(s.index)),
          el('span', { class: 'importer-sent-text' }, s.text),
          el('span', { class: 'muted sm' }, t('imp.tokenCount', { n: (s.tokens || []).length })))),
        doc.sentences.length > 6 ? el('div', { class: 'muted sm' }, t('imp.andMore', { n: doc.sentences.length - 6 })) : null);
    } catch (err) {
      status.textContent = describeError(err); status.className = 'edit-status err';
    }
  }
  ...
  mount(body,
    ...
    sample,
    el('div', { class: 'importer-row' }, draftBtn,
      el('button', { class: 'btn sm ghost', onclick: dryRun }, t('imp.dryRun'))),
    ...
    el('div', { class: 'modal-actions' },
      el('button', {
        class: 'btn sm',
        onclick: async () => {
          // Never adopt a reader that has not run: saving a parser that throws
          // would break importing for this format with no clue why.
          try {
            const probe = sample.value.trim();
            if (probe) await tryImporter(code.value, probe, 'sample.txt');
            setImporter(formatId, code.value);
            toast(t('imp.savedToast', { format: formatId }));
            set({}, 'importers');
            await render(body, formatId);
          } catch (err) {
            status.textContent = t('imp.refusedSave', { err: describeError(err) });
      ...
      current ? el('button', {
        class: 'btn sm ghost',
        onclick: async () => { clearImporter(formatId); toast(t('imp.clearedToast')); await render(body, formatId); },
      }, t('imp.revert')) : null,
      status),
    current
      ? el('div', { class: 'hint' }, t('imp.inUse', { when: new Date(current.updatedAt).toLocaleString() }))
