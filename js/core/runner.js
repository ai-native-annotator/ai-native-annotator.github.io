 ...
 * keyed by (skill, span).
 */
function findRecorded(skillId, span) {
  const idx = state.doc?._trace;
  if (!idx) return null;
  return idx.get(traceKey(skillId, span)) || null;
}
...
export async function runSkillCall({ skillId, span, system, prompt, language }) {
  ...
  return { output, rationale: rationaleOf(output), rawText: text, latencyMs, source: 'live', model };
}
